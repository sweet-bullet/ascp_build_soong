...
func AutogeneratedRroModuleName(ctx android.EarlyModuleContext, moduleName, partition string) string {
	...
	product := strings.Replace(ctx.Config().Getenv("TARGET_PRODUCT"), "lineage_", "aosp_", 1)
	overrideName := strings.Replace(strings.Replace(strings.Replace(moduleName, "lineageos", "aosp", -1), "Lineage", "AOSP", -1), "lineage", "aosp", -1)
	return fmt.Sprintf("%s__%s__auto_generated_rro_%s", overrideName, product, partition)
}
